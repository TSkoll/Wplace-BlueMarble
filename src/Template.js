    ...
    try {
      ...
      for (let y = 0; y < imageHeight; y++) {
        for (let x = 0; x < imageWidth; x++) {
          ...
          if (a === 0) {
            continue;
          } // Ignored transparent pixel
          if (r === 222 && g === 250 && b === 206) {
            deface++;
          }
          const key = this.allowedColorsSet.has(`${r},${g},${b}`)
            ? `${r},${g},${b}`
            : "other";
      ...
      console.warn(
        "Failed to compute required/deface counts. Falling back to total pixels.",
        err
      );
    }
    ...
    for (let pixelY = this.coords[3]; pixelY < imageHeight + this.coords[3]; ) {
      ...
      for (
        ...
        for (let y = 0; y < canvasHeight; y++) {
          for (let x = 0; x < canvasWidth; x++) {
            ...
            if (
              ...
              if ((x + y) % 2 === 0) {
                // Formula for checkerboard pattern
              ...
              // Otherwise only draw the middle pixel
        ...
        const templateTileName = `${(this.coords[0] + Math.floor(pixelX / 1000))
          ...
          .toString()
          .padStart(4, "0")},${(pixelX % 1000).toString().padStart(3, "0")},${(
          pixelY % 1000
        )
          .toString()
          .padStart(3, "0")}`;
        ...
        this.tilePrefixes.add(
          templateTileName.split(",").slice(0, 2).join(",")
        );
        ...
        templateTilesBuffers[templateTileName] =
          uint8ToBase64(canvasBufferBytes); // Stores the buffer

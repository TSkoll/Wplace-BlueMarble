...
if (userSettings?.telemetry == undefined || userSettings?.telemetry > 1) {
  // Increment 1 to retrigger telemetry notice
...
function buildOverlayMain() {
  ...
  overlayMain
    ...
    .addImg(
      ...
      (instance, img) => {
        ...
        img.addEventListener("click", () => {
          ...
          const coordInputs = document.querySelectorAll(
            "#bm-contain-coords input"
          );
    ...
    .addInput(
      {
        ...
        step: 1,
        required: true,
        value: savedCoords.tx ?? "",
      },
      (instance, input) => {
        //if a paste happens on tx, split and format it into other coordinates if possible
        input.addEventListener("paste", (event) => {
          let splitText = (event.clipboardData || window.clipboardData)
            .getData("text")
            .split(" ")
            .filter((n) => n)
            .map(Number)
            .filter((n) => !isNaN(n)); //split and filter all Non Numbers

          if (splitText.length !== 4) {
            // If we don't have 4 clean coordinates, end the function.
            return;
          }
    ...
    // Color filter UI
    .addDiv({
      id: "bm-contain-colorfilter",
      style:
        "max-height: 140px; overflow: auto; border: 1px solid rgba(255,255,255,0.1); padding: 4px; border-radius: 4px; display: none;",
    })
    .addDiv({ style: "display: flex; gap: 6px; margin-bottom: 6px;" })
    .addButton(
      { id: "bm-button-colors-enable-all", textContent: "Enable All" },
      (instance, button) => {
        button.onclick = () => {
    ...
    .buildElement()
    .addButton(
      { id: "bm-button-colors-disable-all", textContent: "Disable All" },
      (instance, button) => {
        button.onclick = () => {
          const t = templateManager.templatesArray[0];
          if (!t?.colorPalette) {
    ...
    .buildElement()
    .addButton(
      { id: "bm-button-create", textContent: "Create" },
      (instance, button) => {
        button.onclick = () => {
    ...
    .addButton(
      ...
      (instance, button) => {
        button.onclick = () => {
          instance.apiManager?.templateManager?.setTemplatesShouldBeDrawn(
            false
          );
    ...
    .buildElement()
    .addSmall({
      textContent: "Made by SwingTheVine",
  ...
  window.buildColorFilterList = function buildColorFilterList() {
    ...
    if (!listContainer || !t?.colorPalette) {
      if (listContainer) {
        listContainer.innerHTML =
          "<small>No template colors to display.</small>";
      }
      return;
    }

    listContainer.innerHTML = "";
    const entries = Object.entries(t.colorPalette).sort(
    ...
    for (const [rgb, meta] of entries) {
      ...
      if (rgb === "other") {
        ...
        try {
          const tMeta =
            templateManager.templatesArray?.[0]?.rgbToMeta?.get(rgb);
      ...
      toggle.addEventListener("change", () => {
        ...
        overlayMain.handleDisplayStatus(
          `${toggle.checked ? "Enabled" : "Disabled"} ${rgb}`
        );
        try {
          const t = templateManager.templatesArray?.[0];
          const key = t?.storageKey;
          if (t && key && templateManager.templatesJSON?.templates?.[key]) {
            templateManager.templatesJSON.templates[key].palette =
              t.colorPalette;
            // persist immediately
            GM.setValue(
              "bmTemplates",
              JSON.stringify(templateManager.templatesJSON)
            );
          }
        } catch (_) {}
      });
...
function buildTelemetryOverlay(overlay) {
  overlay
    .addDiv({
      id: "bm-overlay-telemetry",
      style:
        "top: 0px; left: 0px; width: 100vw; max-width: 100vw; height: 100vh; max-height: 100vh; z-index: 9999;",
    })
    .addDiv({
      id: "bm-contain-all-telemetry",
      style: "display: flex; flex-direction: column; align-items: center;",
    })
    .addDiv({ id: "bm-contain-header-telemetry", style: "margin-top: 10%;" })
    .addHeader(1, { textContent: `${name} Telemetry` })
    .buildElement()
    .buildElement()

    .addDiv({
      id: "bm-contain-telemetry",
    ...
    .addButton(
      { id: "bm-button-telemetry-more", textContent: "More Information" },
      (instance, button) => {
        button.onclick = () => {
          window.open(
            "https://github.com/SwingTheVine/Wplace-TelemetryServer#telemetry-data",
            "_blank",
            "noopener noreferrer"
          );
        };
    ...
    .buildElement()
    .addButton(
      { id: "bm-button-telemetry-disable", textContent: "Disable Telemetry" },
      (instance, button) => {
        button.onclick = () => {
          const userSettings = JSON.parse(GM_getValue("bmUserSettings", "{}"));
    ...
    .addP({
      textContent:
        'You can disable telemetry by pressing the "Disable" button below.',
    })
    .buildElement()
    .buildElement()
    .buildElement()
...
function buildOverlayTabTemplate() {
  overlayTabTemplate
    .addDiv({ id: "bm-tab-template", style: "top: 20%; left: 10%;" })
    .addDiv()
    .addDiv({ className: "bm-dragbar" })
    .buildElement()
    .addButton(
      { className: "bm-button-minimize", textContent: "↑" },
      (instance, button) => {
        button.onclick = () => {
